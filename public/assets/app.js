...
const elements = {
	...
	codegraphEdgeFilters: document.querySelector("#codegraph-edge-filters"),
	...
	codegraphRisk: document.querySelector("#codegraph-risk"),
	codegraphSearch: document.querySelector("#codegraph-search"),
	codegraphExportActions: document.querySelector("#codegraph-export-actions"),
	codegraphPathfinder: document.querySelector("#codegraph-pathfinder"),
	codegraphPathFrom: document.querySelector("#codegraph-path-from"),
	codegraphPathTo: document.querySelector("#codegraph-path-to"),
	codegraphPathDirection: document.querySelector("#codegraph-path-direction"),
	codegraphPathFind: document.querySelector("#codegraph-path-find"),
	codegraphPathSwap: document.querySelector("#codegraph-path-swap"),
	codegraphPathClear: document.querySelector("#codegraph-path-clear"),
	codegraphPathStatus: document.querySelector("#codegraph-path-status"),
	codegraphPathResults: document.querySelector("#codegraph-path-results"),
	codegraphDirectoryTree: document.querySelector("#codegraph-directory-tree"),
...
function setRun(run) {
	...
	if (run?.id && run.id !== previousRunId && run.runKind === "codegraph") {
		...
		state.codegraph.activeCyclePaths = [];
		state.codegraph.pathFrom = "";
		state.codegraph.pathTo = "";
		state.codegraph.pathDirection = "forward";
		state.codegraph.pathResult = null;
		state.codegraph.pathLoading = false;
		state.codegraph.pathError = "";
		state.codegraph.activePathIndex = -1;
		state.codegraph.layoutUserSet = false;
		state.codegraph.layout = "cluster";
		state.codegraph.edgeFilters = {
			injects: true,
			calls: true,
			crossing: true,
			tests: false
...
function codeGraphSyncLayoutControl() {
	...
	const onFocus = state.codegraph.mode === "focus";
	if (elements.codegraphLayoutField) elements.codegraphLayoutField.hidden = onOverview;
	if (onOverview) {
		state.codegraph.layout = "cluster";
	} else if (onFocus && !state.codegraph.layoutUserSet) {
		state.codegraph.layout = "radial";
	}
	if (elements.codegraphLayout) {
		const options = elements.codegraphLayout.querySelectorAll("option");
		options.forEach((opt) => {
			if (opt.value === "radial") opt.textContent = onFocus ? "Hub" : "Radial";
			else if (opt.value === "cluster") opt.textContent = "Cluster";
			else if (opt.value === "layer") opt.textContent = "Layer";
		});
		if (elements.codegraphLayout.value !== state.codegraph.layout) {
			elements.codegraphLayout.value = state.codegraph.layout;
		}
	}
}

function codeGraphFilterEdges(edges) {
	const list = Array.isArray(edges) ? edges : [];
	const filters = state.codegraph.edgeFilters || {};
	return list.filter((edge) => {
		const kind = String(edge.kind || edge.label || "").toLowerCase();
		if (kind === "tests" && !filters.tests) return false;
		if (edge.crossing && !filters.crossing) return false;
		if (kind === "injects" && filters.injects === false) return false;
		if (kind === "calls" && filters.calls === false) return false;
		return true;
	});
}

function renderCodeGraphEdgeFilters() {
	const host = elements.codegraphEdgeFilters;
	if (!host) return;
	const depth = state.codegraph.mode || "cluster";
	const show = depth === "file" || depth === "focus";
	host.hidden = !show;
	if (!show) return;
	const filters = state.codegraph.edgeFilters || {};
	const chips = [
	...
	host.innerHTML = chips
		.map((chip) => {
			const on = filters[chip.key] !== false;
			return `<button type="button" class="codegraph-filter-chip${on ? " is-on" : ""}" data-codegraph-filter="${chip.key}" aria-pressed="${on ? "true" : "false"}">${chip.label}</button>`;
		})
		.join("");
}

function codeGraphProjectName() {
	const path = state.activeRun?.projectPath || elements.projectPath?.value || "";
...
function codeGraphActivePath() {
	const paths = Array.isArray(state.codegraph.pathResult?.paths) ? state.codegraph.pathResult.paths : [];
	const index = Number(state.codegraph.activePathIndex);
	return index >= 0 && index < paths.length ? paths[index] : null;
}

function codeGraphPathHighlight(path = codeGraphActivePath()) {
	const hops = Array.isArray(path?.hops) ? path.hops : [];
	if (!hops.length) return {};
	const steps = [hops[0].from].concat(hops.map((hop) => hop.to)).map(codeGraphNormId).filter(Boolean);
	const kinds = hops.map((hop) => String(hop.kind || ""));
	return {
		flowStepSet: steps,
		flowEdgeKinds: kinds
	};
...
function codeGraphFlowHighlightOpts() {
	const pathHighlight = codeGraphPathHighlight();
	if (pathHighlight.flowStepSet?.length) return pathHighlight;
	const cyclePaths = Array.isArray(state.codegraph.activeCyclePaths) ? state.codegraph.activeCyclePaths : [];
	if (cyclePaths.length >= 2) {
		const steps = cyclePaths.map((step) => codeGraphNormId(step)).filter(Boolean);
		if (steps.length >= 2) {
			const closed = steps.concat(steps[0]);
			return { flowStepSet: closed, cycleHighlight: true };
		}
	}
...
async function codeGraphSelectFlow(flowId) {
	...
	state.codegraph.activeCyclePaths = [];
	state.codegraph.activePathIndex = -1;
	renderCodeGraphPathFinder(true);
...
async function codeGraphSelectCycle(paths) {
	const list = (Array.isArray(paths) ? paths : []).map(String).filter(Boolean);
	if (list.length < 2) return;
	const same =
		state.codegraph.activeCyclePaths.length === list.length &&
		state.codegraph.activeCyclePaths.every((path, i) => codeGraphIdsEqual(path, list[i]));
	if (same) {
		state.codegraph.activeCyclePaths = [];
		paintCodeGraphCanvas({ fit: false });
	...
	state.codegraph.activePathIndex = -1;
	renderCodeGraphPathFinder(true);
	state.codegraph.activeCyclePaths = list;
	const focusId = list[0];
	if (state.codegraph.mode === "cluster" || state.codegraph.mode === "overview") {
		await codeGraphOpenFile(focusId, { neighbourhood: true });
		return;
	}
	paintCodeGraphCanvas({ fit: false });
	renderCodeGraphIssues();
...
function codeGraphPathNode(pathOrId) {
	const raw = String(pathOrId || "");
	const hit = codeGraphLookupNode(raw);
	if (hit) {
		return {
			...hit,
			id: codeGraphNormId(hit.id || hit.path || raw),
			path: hit.path || raw
		};
	}
	const lower = raw.toLowerCase();
	const contextual = lower.startsWith("route:") || lower.startsWith("table:") || lower.startsWith("http:") || lower.startsWith("schedule:");
	return {
		id: codeGraphNormId(raw),
		path: raw,
		label: raw,
		kind: contextual ? lower.split(":", 1)[0] : "file",
		role: lower.startsWith("route:") ? "entry" : lower.startsWith("table:") ? "persistence" : "integration",
		layerLabel: lower.startsWith("route:") ? "entry" : lower.startsWith("table:") ? "persistence" : "integration"
	};
}

function codeGraphShowPath(index) {
	const paths = Array.isArray(state.codegraph.pathResult?.paths) ? state.codegraph.pathResult.paths : [];
	const selected = paths[index];
	if (!selected || !Array.isArray(selected.hops) || !selected.hops.length) return;
	state.codegraph.activePathIndex = index;
	state.codegraph.activeFlowId = "";
	state.codegraph.activeCyclePaths = [];
	const nodesById = new Map();
	selected.hops.forEach((hop) => {
		[hop.from, hop.to].forEach((id) => {
			const node = codeGraphPathNode(id);
			if (node.id) nodesById.set(node.id, node);
		});
	});
	state.codegraph.mode = "focus";
	state.codegraph.focusId = codeGraphNormId(selected.hops[0].from);
	state.codegraph.focusSubgraph = {
		focus: selected.hops[0].from,
		pathFinder: true,
		nodes: Array.from(nodesById.values()),
		edges: selected.hops.map((hop) => ({
			from: hop.from,
			to: hop.to,
			sourceFile: hop.from,
			targetFile: hop.to,
			kind: hop.kind || "depends",
			line: hop.line,
	...
	state.codegraph.selectedId = "";
	state.codegraph.selectedEdge = null;
	state.codegraph.layout = "swimlane";
	state.codegraph.layoutUserSet = true;
	paintCodeGraphCanvas({ fit: true });
	renderCodeGraphPathFinder(true);
}

function codeGraphSetPathEndpoint(which, pathOrId) {
	if (which !== "from" && which !== "to") return;
	const node = codeGraphLookupNode(pathOrId);
	const value = String(node?.path || node?.id || pathOrId || "");
	if (!value) return;
	state.codegraph[which === "from" ? "pathFrom" : "pathTo"] = value;
	state.codegraph.pathResult = null;
	state.codegraph.pathError = "";
	state.codegraph.activePathIndex = -1;
	renderCodeGraphPathFinder(true);
...
function codeGraphClearPath({ resetView = true } = {}) {
	const wasPathView = !!state.codegraph.focusSubgraph?.pathFinder;
	state.codegraph.pathFrom = "";
	state.codegraph.pathTo = "";
	state.codegraph.pathResult = null;
	state.codegraph.pathError = "";
...
async function codeGraphFindPaths() {
	const runId = state.activeRun?.id || "";
	const from = String(state.codegraph.pathFrom || "");
	const to = String(state.codegraph.pathTo || "");
	if (!runId || !from || !to || state.codegraph.pathLoading) return;
	state.codegraph.pathLoading = true;
	state.codegraph.pathError = "";
	state.codegraph.pathResult = null;
	state.codegraph.activePathIndex = -1;
	renderCodeGraphPathFinder(true);
	try {
		const params = new URLSearchParams({
			from,
			to,
			direction: state.codegraph.pathDirection || "forward",
			limit: "3"
		});
		const payload = await request(`/api/v1/runs/${encodeURIComponent(runId)}/codegraph/paths?${params.toString()}`);
		state.codegraph.pathResult = payload.data || { paths: [], truncated: false, reason: "unreachable" };
		if (Array.isArray(state.codegraph.pathResult.paths) && state.codegraph.pathResult.paths.length) {
			codeGraphShowPath(0);
		}
	} catch (error) {
		state.codegraph.pathError = error.message || "Could not find a dependency path.";
	} finally {
		state.codegraph.pathLoading = false;
		renderCodeGraphPathFinder(true);
	}
...
function renderCodeGraphPathFinder(hasSnapshot) {
	const host = elements.codegraphPathfinder;
	if (!host) return;
	host.hidden = !hasSnapshot;
	if (!hasSnapshot) return;
	const from = String(state.codegraph.pathFrom || "");
	const to = String(state.codegraph.pathTo || "");
	if (elements.codegraphPathFrom) {
		elements.codegraphPathFrom.textContent = from || "Choose a node";
		elements.codegraphPathFrom.title = from;
	}
	if (elements.codegraphPathTo) {
		elements.codegraphPathTo.textContent = to || "Choose a node";
		elements.codegraphPathTo.title = to;
	}
	if (elements.codegraphPathDirection && elements.codegraphPathDirection.value !== state.codegraph.pathDirection) {
		elements.codegraphPathDirection.value = state.codegraph.pathDirection || "forward";
	}
	if (elements.codegraphPathFind) elements.codegraphPathFind.disabled = !from || !to || state.codegraph.pathLoading;
	if (elements.codegraphPathSwap) elements.codegraphPathSwap.disabled = !from && !to;
	if (elements.codegraphPathClear) elements.codegraphPathClear.disabled = !from && !to && !state.codegraph.pathResult;

	const result = state.codegraph.pathResult;
	const paths = Array.isArray(result?.paths) ? result.paths : [];
	let status = "Select a file, then set it as path start or end.";
	if (from && to) status = "Ready to search bounded directed paths.";
	if (state.codegraph.pathLoading) status = "Searching bounded dependency paths…";
	else if (state.codegraph.pathError) status = state.codegraph.pathError;
	else if (paths.length) status = `${paths.length} path${paths.length === 1 ? "" : "s"} found${result?.truncated ? " · result capped" : ""}.`;
	else if (result?.reason === "unreachable") status = "No directed path found.";
	else if (result?.truncated) status = `Search capped after ${Number(result.exploredNodes || 0)} explored nodes.`;
	if (elements.codegraphPathStatus) elements.codegraphPathStatus.textContent = status;

	const resultsHost = elements.codegraphPathResults;
	if (!resultsHost) return;
	resultsHost.hidden = !paths.length;
	if (!paths.length) {
		resultsHost.innerHTML = "";
		return;
	}
	const active = codeGraphActivePath() || paths[0];
	resultsHost.innerHTML = `<div class="codegraph-path-options">${paths.map((path, index) => {
		const selected = index === Number(state.codegraph.activePathIndex);
		return `<button type="button" class="codegraph-path-option${selected ? " is-active" : ""}" data-codegraph-path-index="${index}" aria-pressed="${selected ? "true" : "false"}">Path ${index + 1} · ${Number(path.hopCount || path.hops?.length || 0)} hops · weight ${escapeHtml(String(path.weight ?? "—"))}</button>`;
	}).join("")}</div><ol class="codegraph-path-hops">${(active.hops || []).map((hop) => {
		const line = Number(hop.line || 0) > 0 ? ` · line ${Number(hop.line)}` : "";
		const provenance = hop.resolution ? ` · ${escapeHtml(String(hop.resolution))}` : "";
		const evidence = hop.evidence ? `<small>${escapeHtml(String(hop.evidence))}</small>` : "";
		return `<li><code>${escapeHtml(String(hop.from || ""))}</code><span>${escapeHtml(String(hop.kind || "depends"))}${line}${provenance}</span><code>${escapeHtml(String(hop.to || ""))}</code>${evidence}</li>`;
	}).join("")}</ol>`;
}
...
function renderCodeGraphMeaningBanner(hasSnapshot) {
	...
	const pending = !!(narrative && narrative.pending);
	...
	const canRetry = !pending && (providerLive || narrativeEnabled);
	const retrying = !!state.codegraph.narrativeRetrying;
	const err = String(narrative?.error || "").trim();
	let text = pending
		? "Meaning briefing arriving — deterministic structure is ready."
	...
	if ((providerLive || narrativeEnabled) && err) {
		...
		text = `Meaning briefing failed — ${short}`;
	} else if (providerLive || narrativeEnabled) {
		text = "Meaning briefing unavailable for this run — retry without re-indexing, or re-run CodeGraph.";
	}
	const retryBtn = canRetry
		? `<button type="button" class="secondary-button codegraph-retry-narrative"${
				retrying ? " disabled" : ""
			}>${retrying ? "Retrying…" : "Retry briefing"}</button>`
		: "";
	host.innerHTML = `<p class="codegraph-banner-text">${escapeHtml(text)}</p>${retryBtn}`;
}

async function codeGraphRetryNarrative() {
	const runId = state.activeRun?.id;
	if (!runId || state.codegraph.narrativeRetrying) return;
	state.codegraph.narrativeRetrying = true;
	renderCodeGraphMeaningBanner(!!(state.codegraph.snapshot && (state.codegraph.snapshot.clusters || state.codegraph.snapshot.nodes)));
	try {
		const payload = await request(`/api/v1/runs/${encodeURIComponent(runId)}/codegraph/narrative`, {
			method: "POST"
		});
		const narrative = payload.data?.narrative;
		if (narrative && typeof narrative === "object") {
			if (state.codegraph.snapshot) {
				state.codegraph.snapshot = Object.assign({}, state.codegraph.snapshot, { narrative });
			}
			if (state.codegraph.result?.codegraph) {
				state.codegraph.result = Object.assign({}, state.codegraph.result, {
					codegraph: Object.assign({}, state.codegraph.result.codegraph, { narrative })
				});
			}
		}
		renderCodeGraph(state.codegraph.result || {});
		...
		if (elements.codegraphFailed) {
			elements.codegraphFailed.hidden = false;
			elements.codegraphFailed.textContent = error.message || "Could not retry meaning briefing.";
		}
		renderCodeGraphMeaningBanner(!!(state.codegraph.snapshot && (state.codegraph.snapshot.clusters || state.codegraph.snapshot.nodes)));
	} finally {
		state.codegraph.narrativeRetrying = false;
		if (!codeGraphMeaningUsed()) {
			renderCodeGraphMeaningBanner(!!(state.codegraph.snapshot && (state.codegraph.snapshot.clusters || state.codegraph.snapshot.nodes)));
		}
	}
}
...
function renderCodeGraphRisk() {
	const host = elements.codegraphRisk;
	if (!host) return;
	const narrative = codeGraphSnapshotNarrative();
	const risks = narrative && narrative.used && Array.isArray(narrative.risk) ? narrative.risk.filter((item) => item && String(item.text || "").trim()) : [];
	host.hidden = !risks.length;
	if (!risks.length) {
		host.innerHTML = "";
		return;
	}
	host.innerHTML = `<p class="codegraph-section-label">Risks surfaced from evidence</p><ul>${risks.slice(0, 8).map((item) => `<li><span class="ai-chip" title="Generated by AI — not evidence">AI</span> ${escapeHtml(String(item.text))}</li>`).join("")}</ul>`;
}

function codeGraphSearchItems(snapshot, query) {
	...
	const items = [];
	nodes.forEach((node) => {
		const haystack = [node.id, node.path, node.role, node.component, node.layer].map((v) => String(v || "").toLowerCase()).join(" ");
		if (haystack.includes(q)) items.push({ type: "node", id: node.id || node.path, label: node.path || node.id, meta: node.role || node.component || "file" });
	});
	clusters.forEach((cluster) => {
		const haystack = [cluster.id, cluster.label, cluster.key, ...(cluster.filePaths || [])].map((v) => String(v || "").toLowerCase()).join(" ");
		if (haystack.includes(q)) items.push({ type: "cluster", id: cluster.id, label: cluster.label || cluster.id, meta: `${cluster.fileCount || 0} files` });
	});
	const seenSymbols = new Set();
	flows.forEach((flow) => {
		const haystack = [flow.id, flow.entryFile, flow.entrySymbol, flow.sinkFile, ...(flow.steps || []), ...(flow.stepSymbols || [])].map((v) => String(v || "").toLowerCase()).join(" ");
		if (haystack.includes(q)) items.push({ type: "node", id: flow.entryFile || flow.id, label: flow.entrySymbol ? `${flow.entryFile}#${flow.entrySymbol}` : flow.entryFile || flow.id, meta: flow.sinkKind || "flow" });
		(flow.stepSymbols || []).forEach((symbol, index) => {
			const name = String(symbol || "").trim();
			const file = String((flow.steps || [])[index] || "").trim();
			const key = `${file.toLowerCase()}\0${name.toLowerCase()}`;
			if (!name || !file || seenSymbols.has(key) || !`${name} ${file}`.toLowerCase().includes(q)) return;
			seenSymbols.add(key);
			items.push({ type: "symbol", id: file, label: name, meta: file });
		});
	});
	return items.slice(0, 60);
}

function codeGraphSymbolsForFile(snapshot, pathOrId) {
	const target = codeGraphNormId(pathOrId);
	const symbols = [];
	const seen = new Set();
	(Array.isArray(snapshot?.flows) ? snapshot.flows : []).forEach((flow) => {
		(flow.steps || []).forEach((step, index) => {
			if (codeGraphNormId(step) !== target) return;
			const symbol = String((flow.stepSymbols || [])[index] || "").trim();
...
/**
 * One whole level from the stored graph, for the levels the snapshot has no
 * shape for: `resource` and `knowledge`.
 *
 * Cached per (run, level) so switching depths does not refetch. `context` is
 * kept alongside `nodes` rather than merged into it, matching the endpoint —
 * the far endpoints of these edges live at other levels and must not be counted
 * as this one.
 */
async function codeGraphLoadLevelCanvas(level) {
	const runId = state.activeRun?.id || "";
	const wanted = String(level || "");
	if (!runId || !wanted) return;
	if (state.codegraph.levelCanvas?.level === wanted && state.codegraph.levelCanvas?.runId === runId) return;
	try {
		const payload = await request(
			`/api/v1/runs/${encodeURIComponent(runId)}/codegraph/graph?level=${encodeURIComponent(wanted)}&limit=200`
		);
		state.codegraph.levelCanvas = { level: wanted, runId, ...(payload.data || {}) };
	} catch (error) {
		state.codegraph.levelCanvas = { level: wanted, runId, nodes: [], edges: [], context: [], unavailable: true };
	}
	paintCodeGraphCanvas({ fit: true });
}
...
function renderCodeGraphDirectoryTree() {
	...
	host.hidden = !directories.length;
	host.innerHTML = directories.slice(0, 80).map((dir) => `<span class="codegraph-tree-row"><strong>${escapeHtml(String(dir.path || "."))}</strong><small>${Number(dir.fileCount || 0)} files · ${Number(dir.symbolCount || 0)} symbols</small></span>`).join("");
}

function syncCodeGraphUrl() {
	if (!window.history?.replaceState || !state.activeRun?.id || state.workspace !== "codegraph") return;
	const params = new URLSearchParams(window.location.search);
	params.set("run", state.activeRun.id);
	params.set("depth", state.codegraph.mode || "cluster");
	if (state.codegraph.clusterId) params.set("cluster", state.codegraph.clusterId); else params.delete("cluster");
	if (state.codegraph.focusId) params.set("focus", state.codegraph.focusId); else params.delete("focus");
	if (state.codegraph.search) params.set("q", state.codegraph.search); else params.delete("q");
...
function renderCodeGraphProjectStrip(view) {
	...
	host.hidden = false;
	const title = document.querySelector("#codegraph-project-title");
	...
	if (blurb) {
		// The stored pitch is produced per cluster shard, so it describes one
		// module — and it replaced the project blurb outright. On this repository
		// that answered "what is this application" with "…centers on a single
		// domain cluster… spans three files and 65 symbols": the project described
		// as one of its own modules, in the first sentence a stranger reads.
		//
		// The structural sentence always leads, because it is about the project and
		// is true without a provider. A module pitch may follow it, labelled with
		// the module it is actually about.
		const structural = copy.blurb || "";
		...
		if (meaningUsed && pitch) {
			...
			const featured = copy.featuredModule ? `Largest module — ${escapeHtml(copy.featuredModule)}: ` : "";
			blurb.innerHTML =
				`<span>${escapeHtml(structural)}</span> ` +
				`<span class="ai-chip" title="Generated by ${provider} — not evidence">AI</span> ` +
				`<span>${featured}${escapeHtml(pitch)}</span>`;
		} else {
			blurb.textContent = structural;
		}
	}
	if (stats) {
		const parts = [`${copy.files} files`, `${copy.modules} modules`];
		if (copy.cycles) parts.push(`${copy.cycles} cycles`);
		if (view?.overviewCapped) {
			parts.push(`showing ${view.shownClusters} of ${view.totalClusters}`);
		}
		stats.textContent = parts.join(" · ");
	}
	if (elements.codegraphOverviewMore) {
		const capped = !!view?.overviewCapped;
		const showingAll = !!state.codegraph.overviewShowAll;
		elements.codegraphOverviewMore.hidden = !capped && !showingAll;
		// "Show all 36 modules" beside 18 drawn cards reads as though 36 are on
		// screen. A disclosure control has to state the cut it is offering to undo.
		const total = view?.totalClusters || copy.modules || 0;
		const shown = view?.shownClusters || 0;
		elements.codegraphOverviewMore.textContent = showingAll
			? `Showing all ${total} — show the top modules only`
			: `Showing ${shown} of ${total} modules — show the rest`;
	}
...
function codeGraphIssueLabel(item, index, tab) {
	const basename = (path) => {
		const p = String(path || "").replace(/\\/g, "/");
		return p.split("/").pop() || p || "—";
	};
	if (Array.isArray(item)) {
		const paths = item.map(String).filter(Boolean);
		return {
			id: paths[0] || `${tab}-${index}`,
			label: paths.map(basename).join(" → ") || `${tab} ${index + 1}`,
			paths
		};
	}
	if (typeof item === "string") {
		return { id: item, label: basename(item), paths: [item] };
	}
	if (!item || typeof item !== "object") {
		return { id: `${tab}-${index}`, label: `${tab} ${index + 1}`, paths: [] };
	}
	const members = [].concat(item.members || item.filePaths || item.nodes || []).filter(Boolean).map(String);
	const path = item.path || item.from || item.to || item.nodeId || "";
	const paths = members.length
		? members
		: [item.path, item.from, item.to, item.id].filter(Boolean).map(String);
	const id = String(item.id || path || members[0] || `${tab}-${index}`);
	let label =
		item.label ||
		item.message ||
		(path ? basename(path) : "") ||
		(members.length ? members.map(basename).join(" → ") : "");
	if (!label) label = `${tab} ${index + 1}`;
	if (tab === "violations" && item.from && item.to) {
		label = `${basename(item.from)} → ${basename(item.to)}${item.kind ? ` (${item.kind})` : ""}`;
	}
	if (tab === "hotspots" && (item.path || item.id)) {
		const score = item.hotspotScore != null ? ` · hot ${Math.round(Number(item.hotspotScore) || 0)}` : "";
		label = `${basename(item.path || item.id)}${score}`;
	}
	return { id, label, paths: paths.length ? paths : [id] };
}

function codeGraphLookupNode(pathOrId) {
	const key = String(pathOrId || "").replace(/\\/g, "/").toLowerCase();
	if (!key) return null;
	const fromLayout = state.codegraph.layoutResult?.nodes?.find((n) => {
		const id = String(n.id || "").replace(/\\/g, "/").toLowerCase();
		const path = String(n.path || "").replace(/\\/g, "/").toLowerCase();
		return id === key || path === key;
	});
	if (fromLayout) return fromLayout;
	const snap = state.codegraph.snapshot || {};
	const fromSnap = (snap.nodes || []).find((n) => {
		const id = String(n.id || "").replace(/\\/g, "/").toLowerCase();
		const path = String(n.path || "").replace(/\\/g, "/").toLowerCase();
		return id === key || path === key;
	});
	...
	return {
		id: String(fromSnap.id || fromSnap.path || pathOrId),
		path: fromSnap.path || fromSnap.id || "",
		label: fromSnap.label || String(fromSnap.path || fromSnap.id || "").replace(/\\/g, "/").split("/").pop(),
		kind: fromSnap.kind || "file",
		clusterId: fromSnap.clusterId || "",
		fanIn: fromSnap.fanIn,
		fanOut: fromSnap.fanOut,
		hotspotScore: fromSnap.hotspotScore,
		inCycle: !!fromSnap.inCycle,
		symbolCount: fromSnap.symbolCount,
		layer: fromSnap.layer,
		layerLabel: typeof fromSnap.layer === "string" ? fromSnap.layer : fromSnap.component || ""
	};
}

async function codeGraphOpenFile(pathOrId, { neighbourhood = false } = {}) {
	const node = codeGraphLookupNode(pathOrId);
	const id = node?.id || pathOrId;
	if (!id) return;
	const onOverview = state.codegraph.mode === "cluster" || state.codegraph.mode === "overview";
	if (onOverview || (state.codegraph.mode === "file" && node?.clusterId && state.codegraph.clusterId !== node.clusterId)) {
		state.codegraph.mode = "file";
		state.codegraph.clusterId = node?.clusterId || state.codegraph.clusterId || "";
		if (!state.codegraph.clusterId) {
			const cluster = (state.codegraph.snapshot?.clusters || []).find((c) =>
				(c.filePaths || []).some(
					(p) => String(p).replace(/\\/g, "/").toLowerCase() === String(id).replace(/\\/g, "/").toLowerCase()
				)
			);
			if (cluster) state.codegraph.clusterId = cluster.id;
		}
		state.codegraph.focusId = "";
		state.codegraph.focusSubgraph = null;
		state.codegraph.selectedEdge = null;
...
async function codeGraphLoadSourceExcerpt(path, line, host) {
	if (!host) return;
	const runId = state.activeRun?.id || "";
	host.hidden = false;
	if (!runId) {
		host.innerHTML = '<p class="field-hint">Run unavailable.</p>';
		return;
	}
	host.innerHTML = '<p class="field-hint">Loading source excerpt…</p>';
	try {
		const params = new URLSearchParams({
			path: String(path || ""),
			line: String(line || 1),
		...
		const payload = await request(`/api/v1/runs/${encodeURIComponent(runId)}/codegraph/source?${params.toString()}`);
		const data = payload?.data || {};
		const lines = Array.isArray(data.lines) ? data.lines : [];
		host.innerHTML = `<div class="codegraph-source-heading">${escapeHtml(data.path || path)}:${escapeHtml(String(data.focusLine || line))}</div><pre>${lines
			.map((item) => `<span class="${item.focus ? "is-focus" : ""}"><b>${escapeHtml(String(item.number || ""))}</b> ${escapeHtml(item.text || "")}</span>`)
			.join("\n")}</pre>`;
	} catch (error) {
		host.innerHTML = `<p class="field-hint">${escapeHtml(error.message || "Could not load source excerpt.")}</p>`;
	}
}
...
function codeGraphInspectorHtml(node) {
	...
	if (node.kind === "edge") {
		...
		const citedLine = Number(node.line || 0);
		const sourceBlock = node.fromPath && citedLine > 0
			? `<div class="codegraph-source-block"><button type="button" class="secondary-button" data-codegraph-source data-codegraph-source-path="${escapeHtml(node.fromPath)}" data-codegraph-source-line="${citedLine}">Show source excerpt</button><div class="codegraph-source-excerpt" data-codegraph-source-host hidden></div></div>`
			: "";
		return `<div class="codegraph-inspector-body"><h3>Dependency</h3><dl class="codegraph-inspector-grid">${rows
			...
			.join("")}</dl>${sourceBlock}<p class="field-hint">Edge evidence from the dependency index.</p></div>`;
	}
	const rows = [
		["Kind", node.kind || "—"],
		["Path", node.path || "—"],
		...
		["Symbols", node.symbolCount != null ? String(node.symbolCount) : ""],
		...
		["Co-change", node.coChange?.changeCount != null ? String(node.coChange.changeCount) : ""],
		["Authors", node.coChange?.authorCount != null ? String(node.coChange.authorCount) : ""],
		["Last touched", node.coChange?.lastTouched || ""]
	].filter(([, value]) => value !== "");
	const narrative = codeGraphNarrative();
	...
	const filesBlock = paths.length
		? `<div class="codegraph-inspector-files"><h4>Files</h4><ul>${paths
				...
				.map((p) => {
					const path = String(p);
					const base = path.replace(/\\/g, "/").split("/").pop() || path;
					return `<li><button type="button" class="codegraph-file-link" data-codegraph-file="${escapeHtml(path)}" title="${escapeHtml(path)}">${escapeHtml(base)}</button></li>`;
				})
				.join("")}${
	...
	const symbols = isFile ? codeGraphSymbolsForFile(state.codegraph.snapshot, node.path || node.id) : [];
	const symbolsBlock = symbols.length
		? `<div class="codegraph-inspector-files"><h4>Flow symbols</h4><ul>${symbols.slice(0, 48).map((symbol) => `<li><code>${escapeHtml(symbol)}</code></li>`).join("")}${symbols.length > 48 ? `<li class="codegraph-inspector-more">+${symbols.length - 48} more</li>` : ""}</ul></div>`
		: "";
	...
	const explore =
		node.kind === "cluster" && onOverview
			...
			: "";
	const neighbourhood =
		isFile && state.codegraph.mode !== "focus"
			? `<button type="button" class="secondary-button codegraph-explore-btn" data-codegraph-neighbourhood="${escapeHtml(node.id || node.path)}">Open neighbourhood</button>`
			: "";
	const labelAction =
		node.kind === "cluster" && node.id && state.activeRun?.id
			? `<button type="button" class="secondary-button codegraph-explore-btn" data-codegraph-label-id="${escapeHtml(node.id)}" data-codegraph-label-value="${escapeHtml(node.label || "")}">Rename module</button>`
			: "";
	const pathEndpoint = node.kind !== "cluster" && node.kind !== "edge" ? String(node.path || node.id || "") : "";
	const pathActions = pathEndpoint
		? `<button type="button" class="secondary-button codegraph-explore-btn" data-codegraph-path-endpoint="from" data-codegraph-path-node="${escapeHtml(pathEndpoint)}">Set path start</button><button type="button" class="secondary-button codegraph-explore-btn" data-codegraph-path-endpoint="to" data-codegraph-path-node="${escapeHtml(pathEndpoint)}">Set path end</button>`
		: "";
	return `<div class="codegraph-inspector-body"><h3>${escapeHtml(node.label || node.id || "Node")}</h3><dl class="codegraph-inspector-grid">${rows
		.map(([k, v]) => `<div><dt>${escapeHtml(k)}</dt><dd title="${escapeHtml(v)}">${escapeHtml(v)}</dd></div>`)
		.join("")}</dl>${detSummary}${aiBlock}${symbolsBlock}${callersBlock}${calleesBlock}${filesBlock}<div class="codegraph-inspector-actions">${explore}${neighbourhood}${labelAction}${pathActions}</div><p class="field-hint">${hint}</p></div>`;
}
...
function renderCodeGraphIssues() {
	...
	elements.codegraphIssueList.innerHTML = rows.slice(0, 80).map((item, index) => {
		const normalized = codeGraphIssueLabel(item, index, tab);
		const cycleActive =
			tab === "cycles" &&
			state.codegraph.activeCyclePaths.length === normalized.paths.length &&
			normalized.paths.every((path, i) => codeGraphIdsEqual(path, state.codegraph.activeCyclePaths[i]));
		const activeClass = cycleActive ? " is-cycle-active" : "";
		return `<button type="button" class="codegraph-issue-item${activeClass}" data-codegraph-issue-tab="${escapeHtml(tab)}" data-codegraph-focus="${escapeHtml(String(normalized.id))}" data-codegraph-paths="${escapeHtml(JSON.stringify(normalized.paths))}" title="${escapeHtml(normalized.label)}">${escapeHtml(normalized.label)}</button>`;
	}).join("");
}
...
/**
 * Every occurrence of a name, bound or not.
 *
 * The only endpoint that can report unresolved references *within* its own
 * answer, because it returns occurrences rather than resolved edges — the other
 * panels can state the run's unresolved count and nothing more.
 */
async function renderCodeGraphReferences() {
	const host = document.querySelector("#codegraph-trace-references");
	const input = document.querySelector("#codegraph-references-symbol");
	if (!host || !input) return;
	const symbol = String(input.value || "").trim();
	if (!symbol) {
		host.innerHTML = `<p class="codegraph-empty-hint">Name a symbol.</p>`;
		return;
	}
	host.innerHTML = `<p class="codegraph-empty-hint">Reading…</p>`;
	try {
		const result = await codeGraphQuery(`references?symbol=${encodeURIComponent(symbol)}&limit=100`);
		const defs = result?.definitions || [];
		const refs = result?.references || [];
		if (!defs.length && !refs.length) {
			host.innerHTML = `<p class="codegraph-empty-hint">No definition or occurrence of <code>${escapeHtml(symbol)}</code> in this run.</p>`;
			return;
		}
		const defRows = defs.map((d) => `<button type="button" class="cg-query-row" data-cg-goto="${escapeHtml(String(d.path || ""))}"><span class="cg-query-kind">def</span><span class="cg-query-label">${escapeHtml(String(d.path))}:${Number(d.line) || 0}</span><span class="cg-query-badge">${escapeHtml(String(d.kind || ""))}</span></button>`).join("");
		const refRows = refs.map((r) => `<button type="button" class="cg-query-row" data-cg-goto="${escapeHtml(String(r.path || ""))}"><span class="cg-query-kind">${escapeHtml(String(r.kind || "ref"))}</span><span class="cg-query-label">${escapeHtml(String(r.path))}:${Number(r.line) || 0}</span><span class="cg-query-badge" data-resolution="${escapeHtml(String(r.resolution || ""))}">${escapeHtml(String(r.resolution || ""))}</span></button>`).join("");
		const unresolved = Number(result?.completeness?.unresolvedInResult) || 0;
		host.innerHTML =
			`${defs.length ? `<h5>Defined ${defs.length}</h5>${defRows}` : ""}` +
			`${refs.length ? `<h5>Referenced ${refs.length}${unresolved ? ` · ${unresolved} unresolved` : ""}</h5>${refRows}` : ""}` +
			codeGraphAccountHtml(result?.completeness);
	} catch (error) {
		host.innerHTML = `<p class="codegraph-empty-hint">Could not read references: ${escapeHtml(error.message || "unavailable")}</p>`;
	}
}

/**
 * A request end to end, as a sequence of evidenced hops.
 *
 * `flows[]` carried the whole chain and nothing rendered it, so the product's
 * central promise — follow a request from its entry to its effect — was the one
 * answer with no view. Each hop shows its kind, the symbol that owns it, and the
 * `file:line` the parser matched; a hop that could not be bound says so rather
 * than being drawn as a join.
 */
async function renderCodeGraphFlows() {
	const host = document.querySelector("#codegraph-trace-flows");
	if (!host) return;
	const selected = codeGraphNormId(String(state.codegraph.selectedId || "")).toLowerCase();
	host.innerHTML = `<p class="codegraph-empty-hint">Reading…</p>`;
	try {
		const list = await codeGraphQuery(`flows?limit=50`);
		const flows = list?.flows || [];
		if (!flows.length) {
			host.innerHTML = `<p class="codegraph-empty-hint">No request flows were traced in this run.</p>`;
			return;
		}
		// Prefer a flow that passes through whatever is selected on the map, so the
		// panel answers "how does a request reach *this*".
		const touching = selected
			? flows.filter((f) => (f.steps || []).some((s) => String(s).toLowerCase().includes(selected.replace(/^(file|symbol|resource):/, ""))))
			: [];
		const shown = (touching.length ? touching : flows).slice(0, 3);
		host.innerHTML = shown.map((flow) => {
			const hops = flow.hops || [];
			const rows = hops.map((h) => {
				const where = Number(h.line) > 0 ? `${escapeHtml(String(h.from))}:${Number(h.line)}` : escapeHtml(String(h.from));
				const symbol = h.symbol ? `<span class="cg-query-badge">${escapeHtml(String(h.symbol))}</span>` : `<span class="cg-query-badge">(file level)</span>`;
				return `<button type="button" class="cg-query-row" data-cg-goto="${escapeHtml(String(h.from))}"><span class="cg-query-kind">${escapeHtml(String(h.kind || ""))}</span><span class="cg-query-label">${where} → ${escapeHtml(String(h.to))}</span>${symbol}<span class="cg-query-badge" data-resolution="${escapeHtml(String(h.resolution || ""))}">${escapeHtml(String(h.resolution || ""))}</span></button>`;
			}).join("");
			return `<h5>${escapeHtml(String(flow.entryFile || ""))} → ${escapeHtml(String(flow.sinkFile || ""))} <small>ends: ${escapeHtml(String(flow.sinkKind || "unknown"))}</small></h5>${rows}`;
		}).join("") + codeGraphAccountHtml(list?.completeness);
	} catch (error) {
		host.innerHTML = `<p class="codegraph-empty-hint">Could not read flows: ${escapeHtml(error.message || "unavailable")}</p>`;
	}
}
...
function refreshCodeGraphQueryPanels() {
	...
	if (tab === "trace") { void renderCodeGraphTrace(); void renderCodeGraphFlows(); }
	if (tab === "assess") {
...
function paintCodeGraphCanvas(options = {}) {
	...
	if (depth === "focus" && state.codegraph.focusSubgraph) {
	...
	} else if (depth === "resource" || depth === "knowledge") {
		// 367 resource nodes with 1,251 edges — routes, tables, config keys, events
		// and outbound integrations — and 100+ knowledge nodes were queryable by API
		// and drawn by nothing. They are the levels that answer "what starts a
		// request", "which settings does this read", "what does this talk to" and
		// "what does this mean", so leaving them off the map left those questions
		// answerable only by curl.
		const level = state.codegraph.levelCanvas;
		const context = level?.context || [];
		// The far endpoints of a resource edge live at other levels; the API returns
		// them separately so they are not counted as this level. Drawing needs both.
		const drawable = {
			nodes: [...(level?.nodes || []), ...context].map((n) => ({
				id: codeGraphNormId(n.id),
				path: n.path || "",
				label: n.symbolName || n.path || n.id,
				role: n.role || n.kind || depth,
				kind: n.kind || depth,
				fanIn: Number(n.fanIn) || 0,
				fanOut: Number(n.fanOut) || 0,
				hotspotScore: Number(n.hotspot) || 0
		...
		view = CG.buildFileView(drawable, { maxNodes: 200, detail: true });
		view.edges = codeGraphFilterEdges(view.edges);
	} else if (depth === "file") {
		...
		const filtered = {
			...
			edges: codeGraphFilterEdges(
				state.codegraph.fileEdgesKey === edgesKey ? state.codegraph.fileEdges || [] : []
			)
		};
	...
	const layoutOpts =
		depth === "cluster" || depth === "overview"
			? { overview: true }
			: { detail: true };
	const flowHighlightOpts = codeGraphFlowHighlightOpts();
	const layout = layoutMode === "layer"
		? CG.layoutLayered(view, layoutOpts)
		: layoutMode === "swimlane"
			? CG.layoutSwimlane(view, Object.assign({}, layoutOpts, { flowStepSet: flowHighlightOpts.flowStepSet || [] }))
		: layoutMode === "radial"
	...
	renderCodeGraphBreadcrumb();
	renderCodeGraphDepthControl();
	renderCodeGraphProjectStrip(view);
}
...
function renderCodeGraph(result = {}) {
	...
	if (elements.codegraphExportActions) {
		elements.codegraphExportActions.hidden = !showChrome;
		elements.codegraphExportActions.querySelectorAll("[data-codegraph-export]").forEach((link) => {
			const format = link.dataset.codegraphExport || "markdown";
			link.href = state.activeRun?.id
				? `/api/v1/runs/${encodeURIComponent(state.activeRun.id)}/export?exportFormat=${encodeURIComponent(format)}`
				: "#";
		});
	}
	renderCodeGraphPathFinder(showChrome);
	...
	if (!showChrome) {
		...
		if (elements.codegraphDirectoryTree) elements.codegraphDirectoryTree.hidden = true;
		if (elements.codegraphPathfinder) elements.codegraphPathfinder.hidden = true;
		if (elements.codegraphExportActions) elements.codegraphExportActions.hidden = true;
...
function openEventStream(run, afterSequence = 0) {
	...
	eventTypes.forEach((type) => source.addEventListener(type, (event) => {
		try {
			...
			if (type === "codegraph.clusters" && state.activeRun?.id === run.id) {
				// Phase one persists the deterministic structure before optional AI;
				// load it immediately so the canvas does not wait for narrative.
				loadResult(run.id).catch(() => {});
			}
			if (payload?.data?.run) {
...
async function resumeRunFromQuery() {
	...
	// Only an explicit run id opens a map. Plain /codegraph is a request to scan
	// something — possibly a different project or directory — so it stays on the
	// form. Silently reopening the newest snapshot makes the page look like it has
	// already run and puts a stale map in front of someone who came to point the
	// tool somewhere new.
	//
	// The discoverability problem that is real — a project with dozens of stored
	// graphs offering no route back to one except the dashboard — is solved by
	// *offering* the saved map, not by loading it. See `offerProjectSnapshot`.
	if (!resumeId) {
		state.didAutoResume = true;
		void offerProjectSnapshot();
		return;
	}
	state.didAutoResume = true;
	try {
...
/**
 * Offer the newest saved map without opening it.
 *
 * `/codegraph` is a request to scan something — often a different project or
 * directory than last time — so the form stays in front. Auto-loading the last
 * snapshot put a stale map in the way of that.
 *
 * The gap this closes is discoverability, not arrival: a project with dozens of
 * stored graphs previously offered no route back to one except the dashboard
 * history table. This states that a map exists and lets the reader choose it.
 *
 * Silent on failure by design: no snapshot, an unreadable one, or a path the
 * allowlist rejects all leave the page exactly as it was. This must never be the
 * reason someone cannot start a run.
 */
async function offerProjectSnapshot() {
	if (state.workspace !== "codegraph" || state.activeRun) return;
	const host = elements.codegraphEmpty;
	...
	try {
		...
		const run = payload?.data?.run;
		if (!run?.id) return;
		// Still the empty state — nothing has loaded — with the saved map offered.
		const built = run.completedAt || run.startedAt || "";
		host.insertAdjacentHTML(
			"beforeend",
			`<p class="codegraph-saved-offer">A saved map of <code>${escapeHtml(projectPath)}</code> exists${built ? ` from ${escapeHtml(formatRelativeTime(built))}` : ""}. ` +
				`<button type="button" class="secondary-button" data-open-saved-map="${escapeHtml(run.id)}">Open saved map</button></p>`
		);
	} catch (error) {
		// A 404 here is the ordinary "no snapshot yet" case, which the empty state
		// already states.
	}
}
...
elements.codegraphSearch?.addEventListener("input", (event) => {
...
elements.codegraphDirectoryTree?.addEventListener("click", (event) => {
	...
	const type = item.dataset.codegraphSearchType || "node";
	if (type === "cluster") {
		state.codegraph.search = "";
		state.codegraph.clusterId = target;
		codeGraphEnterFiles(target, { fit: true });
	} else if (target) {
		state.codegraph.search = "";
...
elements.codegraphDrawer?.addEventListener("click", (event) => {
	...
	if (event.target.closest("#codegraph-references-run")) {
		void renderCodeGraphReferences();
		return;
	}
...
elements.codegraphDrawer?.addEventListener("keydown", (event) => {
	...
	if (event.target.closest("#codegraph-references-symbol")) {
		event.preventDefault();
		void renderCodeGraphReferences();
	}
});
...
elements.codegraphPathDirection?.addEventListener("change", (event) => {
	state.codegraph.pathDirection = event.target.value || "forward";
	state.codegraph.pathResult = null;
	state.codegraph.pathError = "";
	state.codegraph.activePathIndex = -1;
	renderCodeGraphPathFinder(true);
...
elements.codegraphPathFind?.addEventListener("click", () => {
	void codeGraphFindPaths();
});

elements.codegraphPathSwap?.addEventListener("click", () => {
	const from = state.codegraph.pathFrom;
	state.codegraph.pathFrom = state.codegraph.pathTo;
	state.codegraph.pathTo = from;
	state.codegraph.pathResult = null;
	state.codegraph.pathError = "";
	state.codegraph.activePathIndex = -1;
	renderCodeGraphPathFinder(true);
...
elements.codegraphPathClear?.addEventListener("click", () => {
	codeGraphClearPath();
});

elements.codegraphPathResults?.addEventListener("click", (event) => {
	const option = event.target.closest("[data-codegraph-path-index]");
	if (!option) return;
	const index = Number(option.dataset.codegraphPathIndex);
	if (Number.isInteger(index) && index >= 0) codeGraphShowPath(index);
});

elements.codegraphEdgeFilters?.addEventListener("click", (event) => {
	const chip = event.target.closest("[data-codegraph-filter]");
	if (!chip) return;
	const key = chip.dataset.codegraphFilter;
	if (!key || !state.codegraph.edgeFilters || !(key in state.codegraph.edgeFilters)) return;
	state.codegraph.edgeFilters[key] = !state.codegraph.edgeFilters[key];
	paintCodeGraphCanvas({ fit: false });
	renderCodeGraphEdgeFilters();
});

// Explicit: the map opens because someone asked for it, not because the page
// loaded. Routes through the same `?run=` path the history Open button uses.
elements.codegraphEmpty?.addEventListener("click", (event) => {
	const btn = event.target.closest("[data-open-saved-map]");
	if (!btn) return;
	const runId = String(btn.dataset.openSavedMap || "").trim();
	if (!runId) return;
	state.codegraph.openedFromSavedMap = true;
	request(`/api/v1/runs/${encodeURIComponent(runId)}`)
		.then((payload) => watchRun(payload.data))
		.catch((error) => {
			btn.disabled = true;
			btn.textContent = error.message || "Could not open the saved map";
		});
});

elements.codegraphMeaningBanner?.addEventListener("click", (event) => {
	const btn = event.target.closest(".codegraph-retry-narrative");
	if (!btn) return;
	codeGraphRetryNarrative();
});
...
elements.codegraphDepth?.addEventListener("click", (event) => {
	...
	// Whole-level views: the rows come from the levelled tables, not the snapshot,
	// so they load rather than filter.
	if (depth === "resource" || depth === "knowledge") {
		state.codegraph.mode = depth;
		codeGraphLoadLevelCanvas(depth);
		paintCodeGraphCanvas({ fit: true });
		renderCodeGraphDepthControl();
		return;
	}
...
elements.codegraphInspector?.addEventListener("click", (event) => {
	const pathEndpoint = event.target.closest("[data-codegraph-path-endpoint]");
	if (pathEndpoint) {
		codeGraphSetPathEndpoint(
			pathEndpoint.dataset.codegraphPathEndpoint || "",
			pathEndpoint.dataset.codegraphPathNode || ""
		);
		return;
	}
	const labelButton = event.target.closest("[data-codegraph-label-id]");
	if (labelButton) {
		const clusterId = labelButton.dataset.codegraphLabelId || "";
		const current = labelButton.dataset.codegraphLabelValue || "";
		const value = window.prompt("Local module label", current);
		if (value == null || !String(value).trim() || !state.activeRun?.id) return;
		labelButton.disabled = true;
		request(`/api/v1/runs/${encodeURIComponent(state.activeRun.id)}/codegraph/label`, {
			method: "PUT",
			body: JSON.stringify({ clusterId, label: String(value).trim() })
		}).then(() => {
			const cluster = (state.codegraph.snapshot?.clusters || []).find((item) => String(item.id) === String(clusterId));
			if (cluster) cluster.label = String(value).trim();
			const narrative = codeGraphNarrative();
			(narrative?.domains || []).filter((item) => String(item.clusterId) === String(clusterId)).forEach((item) => { item.title = String(value).trim(); });
			(narrative?.summaries || []).filter((item) => String(item.clusterId) === String(clusterId)).forEach((item) => { item.title = String(value).trim(); });
			renderCodeGraph(state.codegraph.result || {});
		}).catch((error) => {
			if (elements.codegraphFailed) {
				elements.codegraphFailed.hidden = false;
				elements.codegraphFailed.textContent = error.message || "Could not save module label.";
			}
		}).finally(() => { labelButton.disabled = false; });
		return;
	}
	const source = event.target.closest("[data-codegraph-source]");
	if (source) {
		const block = source.closest(".codegraph-source-block");
		const host = block?.querySelector("[data-codegraph-source-host]");
		void codeGraphLoadSourceExcerpt(
			source.dataset.codegraphSourcePath || "",
			source.dataset.codegraphSourceLine || "1",
			host
		);
		return;
	}
	...
	const neighbourhood = event.target.closest("[data-codegraph-neighbourhood]");
	if (neighbourhood) {
		const id = neighbourhood.dataset.codegraphNeighbourhood || "";
		if (id) codeGraphOpenFile(id, { neighbourhood: true });
		return;
	}
	const file = event.target.closest("[data-codegraph-file]");
	if (file) {
		const path = file.dataset.codegraphFile || "";
		if (path) codeGraphOpenFile(path, { neighbourhood: true });
	}
});
...
elements.codegraphIssueList?.addEventListener("click", (event) => {
	...
	const tab = item.dataset.codegraphIssueTab || state.codegraph.issueTab || "";
	if (tab === "cycles" && paths.length >= 2) {
		codeGraphSelectCycle(paths);
		return;
	}
	const focusId = item.dataset.codegraphFocus || paths[0] || "";
	if (!focusId) return;
	state.codegraph.activeCyclePaths = [];
	codeGraphOpenFile(focusId, { neighbourhood: true });
});
